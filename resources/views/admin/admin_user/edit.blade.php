    ...
    <div class="card">
        <div class="card-body">
            ...
            <form class="forms-sample">
                ...
                <div class="form-group">
                    <label for="exampleSelectGender">权限分配</label>
                    <select class="form-control" id="exampleSelectGender" name="nid">
                        @foreach($nodes as $k=>$v)
                            <option @if($admin_user->nid == $v->id) selected
                                    @endif value="{{$v->id}}">{{$v->name}}</option>
                        @endforeach
                    </select>
                </div>
                <input type="hidden" name="token" value="{{$admin_user->_token}}">
                <div class="form-group">
                    <label>上传头像</label>
            ...
            <button onclick="edit({{$admin_user->aid}})" class="btn btn-gradient-primary mr-2">修改</button>
        </div>
    </div>
</div>
